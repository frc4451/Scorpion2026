...
public class DriveIOSim implements DriveIO {
  ...
  private PIDController leftPID =
      new PIDController(DriveConstants.kSimKp, DriveConstants.kSimKi, DriveConstants.kSimKd);
  ...
  public void updateInputs(DriveIOInputs inputs) {
    if (closedLoop) {
      double left =
          leftFFVolts
              + leftPID.calculate(
                  sim.getLeftVelocityMetersPerSecond() * DriveConstants.kWheelRadiusMeters);
      double right =
          rightFFVolts
              + rightPID.calculate(
  ...
  public void setVoltage(double leftVolts, double rightVolts) {
    closedLoop = false;
    this.leftAppliedVolts = MathUtil.clamp(leftVolts, -12.0, 12.0);
    this.rightAppliedVolts = MathUtil.clamp(rightVolts, -12.0, 12.0);
  }
