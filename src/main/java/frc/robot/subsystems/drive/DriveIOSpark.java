...
public class DriveIOSpark implements DriveIO {
  /**
   * Creates different objects for all the motors, and encoder and controller objects for the lead
   * motors
   */
  private final SparkMax leftLeader =
      new SparkMax(DriveConstants.kFrontLeftId, DriveConstants.kMotorType);

  private final SparkMax rightLeader =
      new SparkMax(DriveConstants.kFrontRightId, DriveConstants.kMotorType);
  private final SparkMax leftFollower =
      new SparkMax(DriveConstants.kBackLeftId, DriveConstants.kMotorType);
  private final SparkMax rightFollower =
      new SparkMax(DriveConstants.kBackRightId, DriveConstants.kMotorType);
  ...
  /**
   * Configures Motor settings. Try until ok exists because of Rev Hardware bug that makes it so
   * sometimes they just don't apply the config and they just won't work
   */
  @Override
  public void configureMotorSettings(IdleMode mode) {
    SparkBaseConfig config = new SparkMaxConfig();
    config
        .openLoopRampRate(DriveConstants.kRampRateSeconds)
    ...
    config.inverted(DriveConstants.kLeftInverted).follow(leftLeader, false);
    SparkUtil.tryUntilOk(
        leftFollower,
        5,
        () ->
    ...
    config.inverted(DriveConstants.kRightInverted).follow(rightLeader, false);
    SparkUtil.tryUntilOk(
        rightFollower,
        5,
        () ->
    ...
    SparkUtil.tryUntilOk(leftLeader, 5, () -> leftEncoder.setPosition(0));

    SparkUtil.tryUntilOk(rightLeader, 5, () -> rightEncoder.setPosition(0));
  }
