...
public class SuperstructureIOSim implements SuperstructureIO{
    private DCMotorSim feederSim = 
        new DCMotorSim(LinearSystemId.createDCMotorSystem(DCMotor.getCIM(1), 0.004, feederMotorReduction), DCMotor.getCIM(1));
    private DCMotorSim intakeLauncherSim = 
        new DCMotorSim(LinearSystemId.createDCMotorSystem(DCMotor.getCIM(1), 0.004, intakeLauncherMotorReduction), DCMotor.getCIM(1));
    ...
    public void updateInputs(SuperstructureIOInputs inputs) {
        ...
        inputs.feederVelocityRedPerSec = feederSim.getAngularVelocityRadPerSec();
